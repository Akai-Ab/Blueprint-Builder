...
app.post('/api/recommendations', (req, res) => {
  ...
  if (selections.backend?.includes('Node.js')) {
    recommendations.push('Express', 'PostgreSQL', 'Redis');
  }
  if (selections.backend?.includes('Python')) {
...
app.post('/api/validate', (req, res) => {
  ...
  if (!bp.frontend?.length && !bp.backend?.length) {
    issues.push({ type: 'missing', field: 'technologies', message: 'Select at least one technology stack' });
  }
  if (bp.frontend?.includes('Next.js') && bp.hosting && !['Vercel', 'AWS'].some(h => bp.hosting.includes(h))) {
